...
<success_criteria>
[S.M.A.R.T. criteria derived from KERNEL Verification — REQUIRED FORMAT]
- **Specific**: [Exact observable outcome]
- **Measurable**: [Quantifiable or binary]
- **Achievable**: [Within scope/limits]
- **Relevant**: [Aligned with mission]
- **Time-bound**: [Duration/iteration limit]
</success_criteria>
...
- **KERNEL** defines agent behavior (how it operates)
...
These guardrails MUST be present in every refactored agent. Add them if missing from original.

### 1. S.M.A.R.T. Success Criteria (REQUIRED)
Every `<success_criteria>` section must follow S.M.A.R.T. format:

```xml
<success_criteria>
- **Specific**: [Exact observable outcome, not vague]
- **Measurable**: [Quantifiable metric or binary check]
- **Achievable**: [Within agent's capabilities and context limits]
- **Relevant**: [Aligned with agent's core mission]
- **Time-bound**: [Duration limit or iteration cap]
</success_criteria>
```

**Example (paranoid-auditor)**:
```xml
<success_criteria>
- **Specific**: Every finding has file:line reference and CVE/CWE citation
- **Measurable**: Zero false positives for CRITICAL severity
- **Achievable**: Complete audit within context limits; split if >5000 lines
- **Relevant**: Findings map to OWASP Top 10 or documented security standards
- **Time-bound**: 45 minutes max; request extension if needed
</success_criteria>
```

### 2. Uncertainty Protocol (REQUIRED)
```xml
<uncertainty_protocol>
- If requirements are ambiguous, ASK for clarification before proceeding
- Say "I don't know" when lacking sufficient information to proceed accurately
...
### 3. Grounding Requirements (REQUIRED)
```xml
<grounding_requirements>
Before generating output:
...
- Format: `component.ts:42` or `> From sdd.md:15: "exact quote"`
</citation_requirements>
```

### 5. Deterministic Logic Extraction (RECOMMENDED)
Identify repetitive, shell-scriptable logic and extract to `resources/scripts/`:

**Patterns to extract**:
- Input validation (sprint ID format, file existence)
- Feedback file detection and priority checking
- Context size assessment for parallel splitting
- Analytics updates
- Git operations (version tagging, changelog updates)

**Script requirements**:
- Must handle errors gracefully (set -e, error messages)
- Must be idempotent where possible
- Must output machine-parseable results (echo "STATUS_CODE")
</quality_guardrails>
...
- [ ] `index.yaml` has: name, version, description, triggers, examples
- [ ] `SKILL.md` has all required XML sections (see below)
- [ ] `resources/` contains externalized content appropriately

## Required XML Sections in SKILL.md
- [ ] `<objective>` — Single sentence purpose
- [ ] `<kernel_framework>` — All 5 KERNEL sections (Task, Context, Constraints, Verification, Reproducibility)
- [ ] `<uncertainty_protocol>` — Permission to say "I don't know"
- [ ] `<grounding_requirements>` — Source document citation rules
- [ ] `<workflow>` — Phase-by-phase execution
- [ ] `<output_format>` — Exact deliverable structure
- [ ] `<success_criteria>` — **S.M.A.R.T. format required**

## Quality Guardrails
- [ ] S.M.A.R.T. criteria are Specific, Measurable, Achievable, Relevant, Time-bound
- [ ] Uncertainty protocol includes "ask for clarification" instruction
- [ ] Grounding requires file:line citations
- [ ] Citation requirements specify format

## Loa-Specific Patterns
- [ ] Parallel execution thresholds match original agent
- [ ] All file paths match Loa conventions (`loa-grimoire/...`)
- [ ] Feedback loop priority preserved (audit feedback → engineer feedback)
- [ ] Verdict patterns preserved ("All good", "CHANGES_REQUIRED", etc.)

## Externalization
- [ ] Bibliography URLs are absolute (not relative)
- [ ] Checklists >20 items moved to `resources/REFERENCE.md`
- [ ] Bash logic moved to `resources/scripts/`
- [ ] Scripts handle errors gracefully (`set -e`, exit codes)
</self_check>